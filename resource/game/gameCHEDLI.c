...
void initPlayer(Personnage *player){
  player->src = IMG_Load("materials/sprites/game/ch-y9.png");
  if(player->src == NULL){
...
void initImages(Personnage* player, Map *map, Object* entity, HUD *hud, NPC* npc){
  ...
  initObject(&entity[4], "materials/maps/topObject.png", entity[4].pos.x, entity[4].pos.y, entity[4].posprite.x, entity[4].posprite.y, entity[4].lvl);
  ...
  initObject(&entity[8], "materials/maps/elevatorDoor.png", entity[8].pos.x, entity[8].pos.y, entity[8].posprite.x, entity[8].posprite.y, entity[8].lvl);
  map->present = IMG_Load("materials/maps/map.png");
  if(map->present == NULL){
    printf("Unable to load present map: %s\n", SDL_GetError());
    exit(EXIT_FAILURE);
  }
  map->future = IMG_Load("materials/maps/map.png");
  if(map->future == NULL){
    printf("Unable to load future map: %s\n", SDL_GetError());
    exit(EXIT_FAILURE);
  }
  map->collisionP = IMG_Load("materials/maps/collisionP.png");
  if(map->collisionP == NULL){
    printf("Unable to load collision for present map: %s\n", SDL_GetError());
    exit(EXIT_FAILURE);
  }
...
void savegame(Map map,Personnage player,Object *entity, NPC *npc)
{
  ...
  if(f != NULL){
    ...
    fwrite(entity, sizeof(Object), 10, f);
...
int AffichageJeuSingleplayer(SDL_Surface* screen, Personnage player, Map map, Object* entity, HUD hud, keybinds *kb, NPC* npc,Background pauseBackground,Background settingsBackground, Button *button,Mix_Chunk *SoundButtonHover, Mix_Chunk *SoundButtonClick, Text txt){
  ...
  int tempsPrec = 0, tempsAct = 0, tPr = 0, tAc = 0, oTime = 0, cTime = 135, startTimer = 0, doortimer = 2, nT = 60, oT = 2;
  Map mapFile;
  SDL_Event event;
...
void loadgame(Personnage* player, Map *map, Object* entity, HUD *hud, NPC* npc){
  ...
  if(f != NULL){
    ...
    fread(entity, sizeof(Object), 10, f);
